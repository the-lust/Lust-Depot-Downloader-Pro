...
class Program
{
    ...
    // FIX: All options as static readonly fields.
    // System.CommandLine resolves values only for the SAME instance that was
    // registered. Creating a new instance in GetValueForOption() always returns null.
    private static readonly Option<uint>    OptApp            = new(new[]{"--app","-a"},            ()=>0u,                       "AppID to download");
    private static readonly Option<uint?>   OptDepot          = new(new[]{"--depot","-d"},                                        "DepotID (downloads all if omitted)");
    private static readonly Option<ulong?>  OptManifest       = new(new[]{"--manifest","-m"},                                     "Manifest ID (requires --depot)");
    private static readonly Option<string>  OptBranch         = new(new[]{"--branch","-b"},         ()=>"public",                 "Branch (default: public)");
    ...
    private static readonly Option<bool>    OptDebug          = new(new[]{"--debug"},                ()=>false,                   "Debug logging");
    private static readonly Option<string?> OptApiKey         = new(new[]{"--api-key","-key"},                                    "GitHub API key for manifest sources");
    ...
    // FIX: track whether we've already handled a Ctrl+C so it doesn't spam
    ...
    // FIX: Single-fire Ctrl+C — saves checkpoint on first press, exits on second
    private static void OnCancelKeyPress(object? sender, ConsoleCancelEventArgs e)
    {
        e.Cancel = true; // prevent default SIGINT kill

        // First press: pause and save checkpoint
        if (Interlocked.CompareExchange(ref _cancelHandled, 1, 0) == 0)
        {
            AnsiConsole.MarkupLine("\n[yellow]⏸  Pausing... Press Ctrl+C again to force quit.[/]");
            try
            {
                _cts?.Cancel();
    ...
    private static async Task<int> ExecuteDownloadAsync(InvocationContext ctx)
    {
        ...
        Logger.Info("Lusts Depot Downloader Pro v1.0.0 - Starting...");
        ...
        try
        {
            ...
            if (ui != null) await ui.RunDownloadWithProgressAsync(_engine);
            else            await _engine.RunAsync();
        ...
        SteamSession steam, DownloadOptions options, TerminalUI? ui)
    {
        ...
        ui?.ShowStatus($"Downloading to: {outputPath}");

        var session = await builder.BuildAsync(outputPath);

        // FIX: When --resume is passed, point CheckpointPath at the existing
        // checkpoint file so DownloadEngine's Checkpoint.Load() picks up the
        // already-completed chunks.  The old code called
        // DownloadSession.LoadFromCheckpoint() which tried to deserialize the
        // checkpoint file as a full DownloadSession — but the checkpoint file
        // only stores CompletedChunks/LastSaved, so AppId came back 0 and
        // OutputDir came back empty, causing the "empty string path" crash.
    ...
    {
        if (_cts is { IsCancellationRequested: false }) { _cts.Cancel(); AnsiConsole.MarkupLine("[yellow]⏸  Paused[/]"); }
        else AnsiConsole.MarkupLine("[yellow]ℹ  No active download[/]");
    }
